...
export default function BookingStep2API({
  ...
  const [addOnsByPackage, setAddOnsByPackage] = useState<{ [packageId: string]: AddOn[] }>(initialAddOns);
  const [ticketAddOnsByPackage, setTicketAddOnsByPackage] = useState<{ [packageId: string]: TicketAddOns }>(initialTicketAddOns);
  const [ticketChoicesByPackage, setTicketChoicesByPackage] = useState<{ [packageId: string]: TicketChoices }>(initialTicketChoices);
    ...
    getAttractions()
      .then((attractions) => {
        ...
        const mapped: Package[] = attractions
          .map((attr) => {
          const existing = selectedPackages.find((p) => p.id === String(attr.id));
          const hasSelectable = attr.selectable_attractions && attr.selectable_attractions > 0;
          ...
          return {
            ...
            image: attr.image_path || local?.image_path || cardImagePlaceholder,
            badge: attr.name,
            ...
            choices: hasSelectable
              ...
              : (local?.choices ?? attr.choices ?? []),
            comboSelectCount: hasSelectable ? attr.selectable_attractions : undefined,
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center py-16 text-gray-400 font-satoshi text-sm">
        Loading packages...
      </div>
    );
  }
